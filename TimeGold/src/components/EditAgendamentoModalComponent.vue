...
export default {
      ...
      handler(newAgendamento) {
        if (newAgendamento && newAgendamento.id) {
          this.form = {
            ...
            scheduledDate: newAgendamento.scheduledDate 
              ? format(parseISO(newAgendamento.scheduledDate), "yyyy-MM-dd'T'HH:mm") 
              : '',
  ...
  methods: {
    async carregarProcedimentos() {
      try {
        if (!this.form.enterpriseId) return;
        const response = await schedulingTypeService.listarTipoAgendamento(1, 100, '', this.form.enterpriseId);
        this.procedimentos = response.items ? response.items : response;
        if (this.form.schedulingTypeId) {
          const existente = this.procedimentos.find(p => p.id === this.form.schedulingTypeId);
          if (!existente) {
            this.procedimentos.push({
              id: this.form.schedulingTypeId,
              name: this.form.procedimento || 'Procedimento antigo',
              value: 0
            });
          }
        }
        ...
        Swal.fire({
          ...
          text: 'Não foi possível carregar os procedimentos. Tente novamente.',
        })
      }
    },
    ...
    async salvarEdicao() {
      ...
      const scheduledDateISO = new Date(this.form.scheduledDate).toISOString();
      ...
      try {
        ...
        Swal.fire({
          ...
          text: this.apiErrors.map(e => e.message).join(', '),
        })
      } finally {
        this.isLoading = false;
      }
